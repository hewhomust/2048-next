...
import { Maybe, Either, Left, IO } from "monet"
...
const newTile = (board, maxID) =>
  IO.of(maxID ? maxID + 1 : 1).chain((nextID) =>
    IO.of(listToGrid(board)).chain((newBoard) =>
      IO.of(findRandomEmptySquare(newBoard))
        .chain(({ row, column }) =>
          newSquareNumber().map((newSquareNumber) =>
            produce(newBoard, (draft) => {
              newBoard[row][column] = {
                id: nextID,
                value: newSquareNumber,
                index: 0,
              }
            })
          )
...
const newSquareNumber = () =>
  IO.of(Math.random()).map((x) => (x < 0.9 ? "2" : "4"))
...
const initializeBoard = (startID) =>
  IO.of([])
    .chain((board) => newTile(board, startID ?? 1))
    .chain((board) => newTile(board, maxID(board)))
